...
*                           FnctName(p_obj);
*
*           (3) 64-bit data types are NOT (or might NOT) be supported by this CPU/compiler.  However, 
*               in order to compile functionality/features requiring 64-bit support; 64-bit data types 
*               MUST be defined even if defined with a precision lower than 64-bits (e.g. with 32-bits 
*               of precision).
*********************************************************************************************************
*/
...
typedef  unsigned  long        CPU_INT64U;                      /* 64-bit unsigned integer NOT supported (see Note #3)  */
typedef    signed  long        CPU_INT64S;                      /* 64-bit   signed integer NOT supported (see Note #3)  */

typedef            float       CPU_FP32;                        /* 32-bit floating point                                */
typedef            float       CPU_FP64;                        /* 64-bit floating point   NOT supported (see Note #3)  */
...
typedef  volatile  CPU_INT64U  CPU_REG64;                       /* 64-bit register         NOT supported (see Note #3)  */
...
typedef  CPU_INT16U             CPU_STK;                        /* Defines CPU stack word size (in octets).             */
...
#define  CPU_INT_DIS()         do { __disable_interrupt(); } while (0)  /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { __enable_interrupt();  } while (0)  /* Enable  interrupts.                          */
#endif
...
#define  CPU_INT_EN()          do { __set_interrupt_state(cpu_sr);    } while (0)
#endif
